...
%==============================================================================
\section{系统模型与不完美 CSI}
%==============================================================================

物理建模的第二步：刻画通信/感知信号在不完美 CSI 下的传输模型。
...
物理建模的第三步：从信号模型出发，推导最坏情况 SINR 等原始非凸表达式。
...
%==============================================================================
\section{SDP 松弛与全局变量重构（Plate II: 数学重构）}
%==============================================================================

物理建模给出极度非凸的 (P1)，本节给出七步凸化推导链将其化为标准凸 SDP (P3)。
...
对于总功率最小化问题，SDR 提供原问题下界 $P_{\text{SDR}}^* \leq P_{\text{original}}^*$。需要说明的是，per-user SINR + cell-free 协作 + 鲁棒问题**无通用紧致性定理**——多组 multicast $G_k \leq 2$ 紧致（Karipidis et al. 2008）与 robust MISO $N_t \leq 2$ 紧致（Tuan, Apaydin, Lu 2012 *Eurasip JWCN*）都不直接套用。
\end{theorem}

\begin{remark}
在高SNR regime下，SDR近似紧致。对于一般情况，可通过高斯随机化恢复波束——这是 SDR-based beamforming 的标准做法（Luo, Ma, So, Ye, Zhang 2010 *IEEE Signal Process. Mag.* 综述），但**无紧致性能损失上界**，实际工程通常用 $L = 100 \sim 1000$ 候选并选最优。
\end{remark}
...
%==============================================================================
\section{秩一恢复与兜底方案（Plate III: 工程落地）}
%==============================================================================

(P3) 给出协方差矩阵解，本节给出高斯随机化与功率缩放兜底方案。
...
\tcp{步骤4：性能保证}
\lIf{$\rank(\mat{W}_k^*) = 1$}{秩一，直接抽取（无松弛 gap）}
\lIf{$\rank(\mat{W}_k^*) > 1$}{高斯随机化（无紧致性能损失上界）}
\label{alg:rank1}
\end{algorithm}
...
%==============================================================================
\section{闭式解推导（Plate IV: 对照基准）}
%==============================================================================

作为 §III (P3) 的对照基准，给出 ZF + 匹配滤波的次优闭式解。
%==============================================================================
