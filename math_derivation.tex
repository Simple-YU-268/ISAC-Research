...
and described in §逐步凸化 below. Of these, Steps 1--6 are
mathematical transformations of the (P2) variables and constraints;
Step 7 is a preprocessing heuristic on the AP service set and does
not enter the (P3) SDP.

%==============================================================================
\section{逐步凸化}
%==============================================================================

我们对 (P2) 施以**七步变换**（编号 Step 1 至 Step 7，详见下文），逐步将每个非凸源转化为凸形式。前六步为对 (P2) 变量与约束的数学变换，第七步（Step 7）为 AP 服务集的预处理启发式。整体思路是：先以 SDR 牺牲 rank-1 紧致性换取可行域的凸化，再以 S-Procedure 处理半无限鲁棒约束，最后对剩余 PCRB / 感知 SINR / 功率约束做线性化或保留。每步变换都尽量保持严格等价（仅 Step 2 (SDR) 在 cell-free 鲁棒场景下**无通用紧致性定理**——见 §逐步凸化 Step 2 注释；Step 7 为工程启发式，**无理论最优性保证**），并在文中即时注明等价性。
...
对所有 $k \in \mathcal{K}$，信道估计满足 $\|\hat{\vect{h}}_k\|_2 = 1$。该归一化吸收进 LMI 矩阵 (S3.1) 右下角 $\mu_k \epsilon_h^2 \|\hat{\vect{h}}_k\|^2$ 项，使最终 (P3-C1) 公式不含 $\|\hat{\vect{h}}_k\|^2$。等价的未归一化形式见 MASTER\_CONSTRAINTS.md §6。
\end{assumption}
...
(P1-C5) per-AP 功率 & (P3-C4) 线性 & Steps 1,6 & 线性（凸） \\
(P1-C6) PSD & (P3-C6) PSD & 不变 & PSD 锥（凸） \\
(P1-C8) 秩一 & (P3-C5) PSD & Step 2 & PSD 锥（凸，**不紧致**）\\
\bottomrule
\end{tabular}
\end{table}

\textbf{关于 (P1-C4) / (P1-C7) AP 选择：} 这两个 (P1) 约束
(P1-C4) $\sum_m b_{mp} = N_{\text{req}}$ 和 (P1-C7) $b_{mp} \in \{0,1\}$
不进入 (P3) 公式块，而是由 Step 7 启发式\textbf{在 (P3) 求解之前}
预处理：对每个目标 $p$，按大尺度衰落 $\text{PL}(d_{m,p})$ 排序
选取 top-$N_{\text{req}}$ AP，得到固定服务集 $\mathcal{M}_p$；(P3) 在
固定 $\mathcal{M}_p$ 上求解凸 SDP。详见 §逐步凸化 Step 7。该启发式
\textbf{无理论最优性保证}——外层 AP 选择可能不是真正的最优组合
（NP-hard），因此不应在表中把 (P1-C4) 标为"线性（凸）"或对应到
(P3-C4)；(P3-C4) 仅为 (P1-C5) per-AP 功率的 lifted 形式。
