...
for seed in [42, 123, 456]:
    ...
    m = Net()
    o = optim.AdamW(m.parameters(), lr=8e-5, weight_decay=2e-5)
...
print(f"v74 (集成): SINR_min={min(r):.2f}dB, 平均={np.mean(r):.2f}dB, ≥0dB={sum(1 for x in r if x>=0)}/100")

# 保存
torch.save([m.state_dict() for m in models], 'isac_v74.pth')
