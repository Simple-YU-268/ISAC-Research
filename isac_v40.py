...
def train_v40(epochs=3000, bs=64, lr=2e-4):
    model = ISAC_v40()
    ...
    for e in range(epochs):
        H = generate_data(bs)
        y_W, y_Z, y_B = generate_labels_v40(bs)
    ...
    torch.save(model.state_dict(), 'isac_v40.pth')
    print("v40完成!")
    return model

def test_v40(model, n=300):
    model.eval()
    results = []
    for i in range(n):
        H = generate_data(1)
    ...
    print(f"v40: 功率={np.mean([r['Total'] for r in results]):.3f}W ± {np.std([r['Total'] for r in results]):.3f}W, APs={np.mean([r['APs'] for r in results]):.0f}")
    return results
