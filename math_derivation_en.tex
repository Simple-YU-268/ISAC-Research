...
%==============================================================================
\section{Original Problem}\label{sec:p1}
%==============================================================================
...
Note that problem (P2) is non-convex. Specifically, the SINR constraint (P2-C1) involves a fractional quadratic form in $\mat{W}_k$ with semi-infinite worst-case uncertainty (NC2), making the universal quantifier ``$\forall \|\Delta\vect{h}_k\| \leq \epsilon_h \|\hat{\vect{h}}_k\|$'' non-convex; the per-AP power constraint (P2-C4) couples the per-AP beamforming vectors $\vect{w}_{m,k}$ with the sensing covariance $\mat{Z}_m$ (NC5); the PCRB constraint (P2-C3) involves the trace of an inverse Fisher information matrix that depends nonlinearly on $\mat{R}_X$ (NC6); the AP-selection constraint (P2-C8) requires a binary indicator $b_{mp} \in \{0,1\}$ (NC3); and the rank-one constraint (P2-C7) demands a non-convex rank-one manifold (NC4). The PSD constraints (P2-C5)/(P2-C6) and the per-AP power linearization are themselves convex, but the overall problem remains non-convex due to the coupling identified above. In the following section, we apply a six-step convexification chain to handle each non-convexity in turn, ultimately reducing the problem to a standard convex SDP. The seven steps
are numbered Step 1 (lifting) through Step 7 (AP-selection heuristic)
and described in \S\ref{sec:convexification} below. Of these, Steps
1--6 are mathematical transformations of the (P2) variables and
constraints; Step 7 is a preprocessing heuristic on the AP service
set and does not enter the (P3) SDP.

%==============================================================================
\section{Step-by-Step Convexification}\label{sec:convexification}
%==============================================================================

We apply **seven transformations** to (P2) (numbered Step 1 through Step 7
in this section): the first six are mathematical transformations of
the (P2) variables and constraints, and the seventh (Step 7) is a
preprocessing heuristic on the AP service set. The transformations
progressively convert each non-convexity into a convex form. The
overall roadmap is: first use SDR to sacrifice rank-one tightness
for convexification of the feasible set, then apply the S-Procedure
to handle the semi-infinite robust constraints, and finally
linearize or retain the PCRB / sensing SINR / power constraints. Each
transformation preserves strict equivalence when possible (only Step 2
SDR is a tight relaxation with no general theorem for our setting,
and Step 7 is an engineering heuristic), with the equivalence type
annotated inline.
...
\begin{assumption}[Channel normalization]\label{ass:channel_norm}
For all $k \in \mathcal{K}$, the channel estimate satisfies $\|\hat{\vect{h}}_k\|_2 = 1$. This normalization absorbs the $\mu_k \epsilon_h^2 \|\hat{\vect{h}}_k\|^2$ term in the (S3.1) LMI, so the final (P3-C1) form does not carry the $\|\hat{\vect{h}}_k\|^2$ factor. The equivalent un-normalized form is documented in MASTER\_CONSTRAINTS.md §6.
\end{assumption}
...
Combining the seven steps from \S\ref{sec:convexification}, starting from \textbf{(P2)} and dropping the rank-one constraint (P2-C7) and applying the heuristic handling of (P2-C8), we obtain the \textbf{(P3)} convex SDP:
\begin{align}
...
(P1-C8) Rank-one & (P3-C5) PSD & Step 2 & PSD cone (convex, **not tight**) \\
\bottomrule
\end{tabular}
\end{table}

\textbf{On (P1-C4) / (P1-C7) AP selection:} These two (P1) constraints
--- (P1-C4) $\sum_m b_{mp} = N_{\text{req}}$ and (P1-C7) $b_{mp} \in \{0,1\}$ ---
do \textbf{not} enter the (P3) constraint block. They are preprocessed
by the Step 7 heuristic \textbf{before} the (P3) SDP is solved: for
each target $p$, rank APs by large-scale fading $\text{PL}(d_{m,p})$ and
pick the top $N_{\text{req}}$ to determine the fixed service set
$\mathcal{M}_p$. (P3) is then solved on the fixed $\mathcal{M}_p$. See
\S\ref{sec:convexification} Step 7 for the full description. The
heuristic \textbf{carries no theoretical optimality guarantee}---the
outer AP selection may not be the true optimum (NP-hard). The
correspondence table therefore omits (P1-C4)/(P1-C7): they are not
mapped to any (P3-Cx). (P3-C4) is solely the lifted form of (P1-C5)
per-AP power.
