...
print("训练...")
m = Net()
o = optim.AdamW(m.parameters(), lr=2e-5, weight_decay=3e-5)
sched = optim.lr_scheduler.CosineAnnealingLR(o, T_max=10000)
Xt = torch.tensor(X, dtype=torch.float32)
At = torch.tensor(A, dtype=torch.float32)

for e in range(10000):
    o.zero_grad()
    i = torch.randperm(len(X))[:64]
...
torch.save(m.state_dict(), 'isac_v78.pth')
...
for _ in range(300):
    ...
    a = m(torch.tensor(h_input.flatten().reshape(1, -1), dtype=torch.float32)).item()
    w = a * mrt_beam(H, Pmax) + (1 - a) * mmse_beam(H, Pmax, 0.3)
    r.append(sinr(H, w).min())

print(f"v78 (8000样本, 300测试): SINR_min={min(r):.2f}dB, 平均={np.mean(r):.2f}dB, ≥0dB={sum(1 for x in r if x>=0)}/300")
