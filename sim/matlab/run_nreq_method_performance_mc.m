...
addParameter(ip, 'M', 6, @(x) isnumeric(x) && isscalar(x) && x >= 2);
addParameter(ip, 'Nt', 2, @(x) isnumeric(x) && isscalar(x) && x >= 1);
addParameter(ip, 'K', 3, @(x) isnumeric(x) && isscalar(x) && x >= 1);
...
for q = 1:numel(nreq_list)
    for i = 1:numel(seeds)
        ...
        prm = generate_scenario(opt.M,opt.Nt,opt.K,opt.P,opt.N_theta,opt.Pmax_dBm, ...
            'auto','AreaSize',opt.AreaSize,'N_req',nreq,'eps_h',opt.eps_h, ...
            'Gamma_alpha',opt.Gamma_alpha,'seed',seed);
...
output.configuration = struct('M',opt.M,'Nt',opt.Nt,'K',opt.K,'P',opt.P, ...
    'N_theta',opt.N_theta,'AreaSize',opt.AreaSize,'Pmax_dBm',opt.Pmax_dBm, ...
    'eps_h',opt.eps_h,'Gamma_alpha',opt.Gamma_alpha);
