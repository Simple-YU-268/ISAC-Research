...
def generate_labels_v16(n):
    W_labels, Z_labels, B_labels = [], [], []
    
    for i in range(n):
        b = np.random.rand(cfg.M, cfg.P) * 0.3
        if i % 2 == 0:
            for p in range(cfg.P):
                selected = np.random.choice(cfg.M, cfg.N_req, replace=False)
                b[selected, p] = 0.85 + np.random.rand(cfg.N_req) * 0.15
...
def train_v16(epochs=800, bs=64, lr=1e-3):
    model = ISAC_v16()
    opt = optim.AdamW(model.parameters(), lr=lr, weight_decay=1e-4)
    sched = optim.lr_scheduler.CosineAnnealingWarmRestarts(opt, T_0=150, T_mult=2)
    
    t0 = time.time()
    
    for e in range(epochs):
        H = generate_data(bs)
        y_W, y_Z, y_B = generate_labels_v16(bs)
        ...
        # 惩罚偏离30W
        pwr_dev = F.relu(total_pwr - cfg.Pmax).mean() * 30
        pwr_low = F.relu(cfg.Pmax * 0.9 - total_pwr).mean() * 30
        loss = loss + pwr_dev + pwr_low
        
        # 鼓励AP选择
        loss = loss - B.mean() * 0.03
    ...
    torch.save(model.state_dict(), 'isac_v16.pth')
    print(f"v16完成! 用时: {time.time()-t0:.1f}s")
    return model

def test_v16(model, n=20):
    model.eval()
    ...
    print(f"v16: 功率={avg_pwr:.1f}W, APs={avg_aps:.0f}")
    return results

if __name__ == '__main__':
    model = train_v16(epochs=800, bs=64, lr=1e-3)
    test_v16(model, n=20)