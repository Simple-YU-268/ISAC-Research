...
原问题要求$\rank(\mat{W}_k) = 1$（因为$\mat{W}_k = \vect{w}_k \vect{w}_k^H$）。半定松弛（SDR）丢弃秩一约束，仅要求$\mat{W}_k \succeq \mat{0}$。
...
问题(P2)是标准凸SDP，可通过内点法在多项式时间内求解到任意精度。
\end{theorem}
