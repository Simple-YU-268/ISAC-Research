...
\begin{align}
\min_{\{\vect{w}_{m,k}\}, \{\mat{Z}_m\}, \{b_{mp}\}} \quad & \sum_{m=1}^{M} \left( \sum_{k=1}^{K} \norm{\vect{w}_{m,k}}^2 + \tr(\mat{Z}_m) \right) \tag{P1} \\
...
其中：
\begin{itemize}
    \item $\vect{w}_{m,k} \in \mathbb{C}^{N_t}$：AP $m$到用户$k$的通信波束成形向量
    \item $\mat{Z}_m \in \mathbb{C}^{N_t \times N_t}$：AP $m$的感知协方差矩阵
    \item $b_{mp} \in \{0,1\}$：AP $m$是否服务目标$p$的二进制指示变量
    \item $P_{\max}$：单AP最大发射功率
    \item $\gamma_k$：通信用户$k$的SINR门限
    \item $\gamma_S^{\text{PoD}}$：感知检测概率门限
    \item $\Gamma_{\text{Track},p}$：目标$p$的跟踪精度门限（FIM迹约束）
    \item $N_{\text{req}}$：每个目标所需服务AP数
\end{itemize}
...
原问题要求$\text{rank}(\mat{W}_k) = 1$（因为$\mat{W}_k = \vect{w}_k \vect{w}_k^H$）。半定松弛（SDR）丢弃秩一约束，仅要求$\mat{W}_k \succeq \mat{0}$。
...
对于总功率最小化问题，当$K \leq 2$时，SDR紧致，即最优解满足$\text{rank}(\mat{W}_k^*) = 1$。
\end{theorem}
...
\begin{equation}
\begin{bmatrix} \mat{A}_k + \mu_k \mat{I} & \mat{A}_k \hat{\vect{h}}_k \\ \hat{\vect{h}}_k^H \mat{A}_k & \hat{\vect{h}}_k^H \mat{A}_k \hat{\vect{h}}_k - \sigma_c^2 + \mu_k \epsilon_h^2 \end{bmatrix} \succeq \mat{0} \tag{18}
\end{equation}
...
类似地，对于感知信道，LMI形式为：
\begin{equation}
\begin{bmatrix} \frac{1}{\gamma_S^{\text{PoD}}} \mat{Z} + \nu_p \mat{I} & \frac{1}{\gamma_S^{\text{PoD}}} \mat{Z} \hat{\vect{g}}_p \\ \frac{1}{\gamma_S^{\text{PoD}}} \hat{\vect{g}}_p^H \mat{Z} & \frac{1}{\gamma_S^{\text{PoD}}} \hat{\vect{g}}_p^H \mat{Z} \hat{\vect{g}}_p - \sigma_s^2 + \nu_p \epsilon_g^2 \end{bmatrix} \succeq \mat{0} \tag{19}
\end{equation}
...
\begin{itemize}
    \item 若$\text{rank}(\mat{W}_k^*) = 1$：直接用特征值分解恢复$\vect{w}_k$
    \item 若$\text{rank}(\mat{W}_k^*) > 1$：需要高斯随机化提取次优波束
\end{itemize}
...
(18) & 通信S-Procedure LMI & $\begin{bmatrix} \mat{A}_k + \mu_k \mat{I} & \mat{A}_k \hat{\vect{h}}_k \\ \hat{\vect{h}}_k^H \mat{A}_k & \hat{\vect{h}}_k^H \mat{A}_k \hat{\vect{h}}_k - \sigma_c^2 + \mu_k \epsilon_h^2 \end{bmatrix} \succeq \mat{0}$ \\
