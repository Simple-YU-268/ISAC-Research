...
def generate_labels_v38(n):
    W_labels, Z_labels, B_labels = [], [], []
    for i in range(n):
        b = np.random.rand(cfg.M, cfg.P) * 0.027
        for p in range(cfg.P):
            selected = np.random.choice(cfg.M, cfg.N_req, replace=False)
            b[selected, p] = 0.9915 + np.random.rand(cfg.N_req) * 0.0085
...
def train_v38(epochs=3200, bs=64, lr=2.2e-4):
    model = ISAC_v38()
    opt = optim.AdamW(model.parameters(), lr=lr, weight_decay=1.4e-5)
    sched = optim.lr_scheduler.CosineAnnealingWarmRestarts(opt, T_0=650, T_mult=2)
    
    for e in range(epochs):
        H = generate_data(bs)
        y_W, y_Z, y_B = generate_labels_v38(bs)
        Xt = torch.tensor(H.reshape(bs, -1), dtype=torch.float32)
        
        W_ratio, Z_ratio, B = model(Xt)
        
        loss_sup = F.mse_loss(W_ratio.squeeze(), torch.tensor(y_W, dtype=torch.float32).squeeze()) + F.mse_loss(Z_ratio.squeeze(), torch.tensor(y_Z, dtype=torch.float32).squeeze()) + F.mse_loss(B, torch.tensor(y_B, dtype=torch.float32)) * 0.018
        
        W_pwr = (W_ratio.squeeze() * cfg.Pmax).mean()
        Z_pwr = (Z_ratio.squeeze() * cfg.Pmax).mean()
        total_pwr = W_pwr + Z_pwr
        
        loss_pwr = F.relu(total_pwr - cfg.Pmax).mean() * 180 + F.relu(cfg.Pmax * 0.992 - total_pwr).mean() * 130 + F.relu(total_pwr - cfg.Pmax * 1.008).mean() * 80
    ...
    torch.save(model.state_dict(), 'isac_v38.pth')
    print("v38完成!")
    return model

def test_v38(model, n=200):
    model.eval()
    results = []
    for i in range(n):
        H = generate_data(1)
    ...
    print(f"v38: 功率={np.mean([r['Total'] for r in results]):.3f}W ± {np.std([r['Total'] for r in results]):.3f}W, APs={np.mean([r['APs'] for r in results]):.0f}")
    return results
