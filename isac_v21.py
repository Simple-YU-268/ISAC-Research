...
def generate_labels_v21(n):
    W_labels, Z_labels, B_labels = [], [], []
    for i in range(n):
        b = np.random.rand(cfg.M, cfg.P) * 0.1
        for p in range(cfg.P):
            selected = np.random.choice(cfg.M, cfg.N_req, replace=False)
            b[selected, p] = 0.95 + np.random.rand(cfg.N_req) * 0.05
        W_labels.append(0.735)
        Z_labels.append(0.335)
        B_labels.append(b.flatten())
    return np.array(W_labels).reshape(-1,1), np.array(Z_labels).reshape(-1,1), np.array(B_labels)

class ISAC_v21(nn.Module):
    def __init__(self):
        super().__init__()
        hd = cfg.M * cfg.K * cfg.Nt * 2
        self.encoder = nn.Sequential(nn.Linear(hd, 256), nn.LeakyReLU(0.2), nn.BatchNorm1d(256), nn.Dropout(0.03), nn.Linear(256, 128), nn.LeakyReLU(0.2), nn.BatchNorm1d(128), nn.Linear(128, 64))
...
def train_v21(epochs=2000, bs=64, lr=4e-4):
    model = ISAC_v21()
    opt = optim.AdamW(model.parameters(), lr=lr, weight_decay=3e-5)
    sched = optim.lr_scheduler.CosineAnnealingWarmRestarts(opt, T_0=400, T_mult=2)
    for e in range(epochs):
        H = generate_data(bs)
        y_W, y_Z, y_B = generate_labels_v21(bs)
        Xt = torch.tensor(H.reshape(bs, -1), dtype=torch.float32)
        W_ratio, Z_ratio, B = model(Xt)
        loss = F.mse_loss(W_ratio.squeeze(), torch.tensor(y_W, dtype=torch.float32).squeeze()) + F.mse_loss(Z_ratio.squeeze(), torch.tensor(y_Z, dtype=torch.float32).squeeze()) + F.mse_loss(B, torch.tensor(y_B, dtype=torch.float32)) * 0.08
        total_pwr = (W_ratio.squeeze() + Z_ratio.squeeze()) * cfg.Pmax
        loss = loss + F.relu(total_pwr - cfg.Pmax).mean() * 100 + F.relu(cfg.Pmax - total_pwr).mean() * 50
        opt.zero_grad()
        loss.backward()
        torch.nn.utils.clip_grad_norm_(model.parameters(), 1.0)
        opt.step()
        sched.step()
    torch.save(model.state_dict(), 'isac_v21.pth')
    print("v21完成!")
    return model

def test_v21(model, n=50):
    model.eval()
    results = []
    for i in range(n):
        H = generate_data(1)
    ...
    print(f"v21: 功率={np.mean([r['Total'] for r in results]):.2f}W ± {np.std([r['Total'] for r in results]):.2f}W, APs={np.mean([r['APs'] for r in results]):.0f}")
    return results
