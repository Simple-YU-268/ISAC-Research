...
def generate_labels_v30(n):
    W_labels, Z_labels, B_labels = [], [], []
    for i in range(n):
        b = np.random.rand(cfg.M, cfg.P) * 0.015
        for p in range(cfg.P):
            selected = np.random.choice(cfg.M, cfg.N_req, replace=False)
            b[selected, p] = 0.996 + np.random.rand(cfg.N_req) * 0.004
...
def train_v30(epochs=8000, bs=64, lr=1e-5):
    model = ISAC_v30()
    opt = optim.AdamW(model.parameters(), lr=lr, weight_decay=5e-7)
    sched = optim.lr_scheduler.CosineAnnealingWarmRestarts(opt, T_0=1600, T_mult=2)
    for e in range(epochs):
        H = generate_data(bs)
        y_W, y_Z, y_B = generate_labels_v30(bs)
        Xt = torch.tensor(H.reshape(bs, -1), dtype=torch.float32)
        W_ratio, Z_ratio, B = model(Xt)
        loss = F.mse_loss(W_ratio.squeeze(), torch.tensor(y_W, dtype=torch.float32).squeeze()) + F.mse_loss(Z_ratio.squeeze(), torch.tensor(y_Z, dtype=torch.float32).squeeze()) + F.mse_loss(B, torch.tensor(y_B, dtype=torch.float32)) * 0.002
        total_pwr = (W_ratio.squeeze() + Z_ratio.squeeze()) * cfg.Pmax
        loss = loss + F.relu(total_pwr - cfg.Pmax).mean() * 800 + F.relu(cfg.Pmax - total_pwr).mean() * 600
        opt.zero_grad()
        loss.backward()
        torch.nn.utils.clip_grad_norm_(model.parameters(), 1.0)
        opt.step()
        sched.step()
    torch.save(model.state_dict(), 'isac_v30.pth')
    print("v30完成!")
    return model

def test_v30(model, n=100):
    model.eval()
    results = []
    for i in range(n):
        H = generate_data(1)
    ...
    print(f"v30: 功率={np.mean([r['Total'] for r in results]):.3f}W ± {np.std([r['Total'] for r in results]):.3f}W, APs={np.mean([r['APs'] for r in results]):.0f}")
    return results
