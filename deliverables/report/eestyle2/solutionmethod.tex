% Original mathematical derivation copied from
% ../math_derivation_tex_v10/math_derivation_en.tex (article scaffold omitted).
\chapter{Optimization Formulation and Solution}\label{ch:solution_method}

Chapter~\ref{ch:system_model} identifies the physical allocation question:
minimize total transmit power while preserving globally cooperative
communication, target-specific sensing quality, and AP-level implementability.
This chapter keeps that interpretation visible throughout the transformation.
Problem (P1) states the original binary physical design; (P2) lifts the
beamforming variables and relaxes only the binary box for optimization; and
(P3) collects the robust and tracking LMIs with the DC penalties used for
candidate recovery. Thus, an LMI-feasible point is an intermediate optimization
object, while a recovered design is accepted only after the physical checks
used in Chapter~\ref{ch:results}.
...
& \min_{\|\Delta\vect{h}_k\| \leq \epsilon_h \|\hat{\vect{h}}_k\|}
  \frac{|\vect{h}_k^H \vect{w}_k|^2}
  {\sum_{j \neq k} |\vect{h}_k^H \vect{w}_j|^2
   + \vect{h}_k^H \big(\textstyle\sum_p \mat{S}_p\big) \vect{h}_k
   + \sigma_c^2}
...
\text{s.t.} \quad
& \tr\big(\hat{\mat{H}}_k(\Delta\vect{h}_k) \mat{W}_k\big)
  - \gamma_k \sum_{j \neq k}
    \tr\big(\hat{\mat{H}}_k(\Delta\vect{h}_k) \mat{W}_j\big) \notag\\
&\quad - \gamma_k \tr\!\Big(\hat{\mat{H}}_k(\Delta\vect{h}_k)
    \textstyle\sum_p \mat{S}_p\Big)
  \geq \gamma_k \sigma_c^2,
  \quad \forall \|\Delta\vect{h}_k\| \leq \epsilon_h \|\hat{\vect{h}}_k\|,
  \ \forall k, \tag{P2-C1} \\
...
The feasible region of (P3) is convex but its objective is DC, so obtaining a
global optimum directly is hard. A KKT stationary point is a candidate for
recovery, not by itself a certificate of a binary rank-one physical design.
This section designs a dual DC penalty SCA framework: the concave term
$-\eta_b b_{mp}^2$ in the (P3) objective and the concave term
$-\eta_{\text{rank}} \lambda_{\max}(\mat{W}_k)$ (corresponding to the missing
rank-1 property) are linearized simultaneously within the same SCA loop, and
each iteration solves a standard convex SDP. This section also presents Gaussian
randomization / leading-eigenvector extraction as a baseline post-processing.
...
\section{Link to Physical Evaluation}\label{sec:solution_to_results}

The convexification chain represents the robust SINR and PCRB conditions by
LMIs in the lifted variables, while the dual DC procedure drives the rank and
binary deficiencies toward zero. Neither a finite penalty nor a stationary
point alone proves exact recovery or global optimality. The final step is
therefore candidate-specific: recover or re-solve the binary topology, extract
the beamformers, and directly validate (P1-C1)--(P1-C7').
Chapter~\ref{ch:results} uses this distinction to compare methods by physical
feasibility first and conditional performance second.
