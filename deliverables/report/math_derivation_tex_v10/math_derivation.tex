...
\Ensure 秩一（或近似秩一）波束协方差 $\{\mat{W}_k^\star\}$、二元 AP-target 分配 $\{b_{mp}^\star \in \{0,1\}\}$、感知协方差 $\{\mat{S}_p^\star\}$。
...
\]
设第 $t$ 次迭代的最优解为 $(\{\mat{W}_k^{(t+1)}\}, \{b_{mp}^{(t+1)}\}, \{\mat{S}_p^{(t+1)}\}, \ldots)$。真实惩罚目标定义为
\[
...
\paragraph{关键不等式 (2).} 取迭代点 $(\{\mat{W}_k^{(t)}\}, \{b_{mp}^{(t)}\})$ 作为第 $t$ 次子问题的可行点（约束 (P3-C1)--(P3-C8) 在迭代点处仍满足，因 (P3-C1)--(P3-C8) 关于 $(\mat{W}_k, \{\mat{S}_p\}, b_{mp})$ 凸、(P3-C9) box 在迭代点满足），其子问题目标值为
\begin{align*}
...
\paragraph{收敛性结论.} 真实目标 $\tilde{P}^{(t)}$ 单调非增、且关于 $(\{\mat{W}_k\}, \{\mat{S}_p\}, \{b_{mp}\})$ 的物理下界为 0（功率非负、惩罚非负），故由单调收敛定理 $\{\tilde{P}^{(t)}\}$ 收敛。
