...
%       'sensing_min_power_fraction' - minimum dedicated sensing power of
%                         each selected AP-target pair as a fraction of Pmax
%                         (default: 0.01; set 0 to recover authorization-only)
...
%       'AP_pos'        - optional M-by-2 AP coordinates in meters
%       'UE_pos'        - optional K-by-2 UE coordinates in meters
%       'Target_pos'    - optional P-by-2 target coordinates in meters
%       'Target_pred_pos' - optional P-by-2 target prediction coordinates
%       'target_prediction_std' - one-sigma target prediction error in m
%
...
addParameter(p, 'sensing_min_power_fraction', 0.01, @(x) isnumeric(x) && isscalar(x) && x >= 0 && x <= 1);
...
addParameter(p, 'AP_pos', [], @isnumeric);
addParameter(p, 'UE_pos', [], @isnumeric);
addParameter(p, 'Target_pos', [], @isnumeric);
addParameter(p, 'Target_pred_pos', [], @isnumeric);
addParameter(p, 'target_prediction_std', 15, @isnumeric);
parse(p, varargin{:});
opt = p.Results;
...
if ~isempty(opt.AP_pos)
    assert(isequal(size(opt.AP_pos), [M, 2]), 'AP_pos must be M-by-2.');
    AP_pos = opt.AP_pos;
end
if ~isempty(opt.UE_pos)
    assert(isequal(size(opt.UE_pos), [K, 2]), 'UE_pos must be K-by-2.');
    UE_pos = opt.UE_pos;
end
if ~isempty(opt.Target_pos)
    assert(isequal(size(opt.Target_pos), [P, 2]), 'Target_pos must be P-by-2.');
    Target_pos = opt.Target_pos;
end
if ~isempty(opt.Target_pred_pos)
    assert(isequal(size(opt.Target_pred_pos), [P, 2]), ...
        'Target_pred_pos must be P-by-2.');
    Target_pred_pos = opt.Target_pred_pos;
else
    Target_pred_pos = Target_pos + opt.target_prediction_std * randn(P, 2);
    Target_pred_pos = max(0, min(AreaSize, Target_pred_pos));
end
...
prm.Target_pred_pos = Target_pred_pos;
...
prm.Pmax = Pmax;
prm.sensing_min_power = opt.sensing_min_power_fraction * Pmax;
prm.sensing_min_power_fraction = opt.sensing_min_power_fraction;
