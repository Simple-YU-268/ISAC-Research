...
prm.Pmax = Pmax;
% A selected AP-target pair must radiate a nonzero dedicated sensing signal.
% Set to zero only for the legacy authorization-only association model.
prm.sensing_min_power_fraction = 0.01;
prm.sensing_min_power = prm.sensing_min_power_fraction * Pmax;
prm.N_req = N_req;
prm.N = N;
prm.M = M;
prm.K = K;
