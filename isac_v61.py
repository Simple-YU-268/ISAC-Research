...
def generate_mmse_data(n_samples):
    ...
    for _ in range(n_samples):
        ...
        # MMSE with better regularization
        H_stack = np.vstack([H_complex[m, :, :] for m in range(M)])
        sigma2 = 0.5  # Higher regularization for stability
...
def train_v61(epochs=4000, bs=32, lr=8e-5):
    model = ISAC_v61()
    ...
    for e in range(epochs):
        ...
        loss_sup = F.mse_loss(p, Pt[idx]) * 40
        
        W_pwr = p.sum(dim=1) * 2.5
        Z_pwr = (z * b).sum(dim=(1,2)) * 2.5
        total_pwr = W_pwr + Z_pwr
        
        loss_pwr = F.relu(total_pwr - Pmax).mean() * 500 + F.relu(Pmax * 0.97 - total_pwr).mean() * 400
    ...
    torch.save(model.state_dict(), 'isac_v61.pth')
    print("v61完成!")
    return model

def test_v61(model, n=100):
    model.eval()
    results = []
    ...
    print(f"v61: 功率={np.mean([r['Total'] for r in results]):.2f}W")
