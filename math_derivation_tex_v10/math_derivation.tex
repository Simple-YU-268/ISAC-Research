...
\paragraph{以目标为中心的感知集群。}
二元变量 $b_{mp}\in\{0,1\}$ 表示 AP $m$ 是否属于目标 $p$ 的感知集群。当 $b_{mp}=1$ 时，AP $m$ 获取的目标 $p$ 回波观测被传送至中央处理器，并纳入目标 $p$ 的联合参数估计。当 $b_{mp}=0$ 时，该观测不参与估计。该指示变量不控制 AP $m$ 的发射功率；通信波束成形保持全网协同。每个 AP 的发射硬件约束为
\begin{equation}
    \tr(\mat{E}_m\mat{R}_X)\leq P_{\max},\qquad \forall m\in\mathcal{M}.
\end{equation}
约束 $\sum_m b_{mp}=N_{\mathrm{req}}$ 给出目标 $p$ 的固定感知集群规模，进而限定接收、前传和处理资源。若 $N_{\mathrm{req}}$ 表示上限而非固定预算，应使用 $\leq N_{\mathrm{req}}$，并显式引入前传或处理代价。

在该定义下，Fisher 信息必须按照接收 AP 分解。令 $\mat{D}_{mp}$ 表示 AP $m$ 获取的目标 $p$ 回波观测关于目标状态的导数，则集群 FIM 定义为
...
\end{equation}
因此，全体 AP 均可照射目标，而只有被选中的接收 AP 向该目标的 PCRB 提供信息。项 $b_{mp}\mat{J}_{mp}(\mat{R}_X)$ 耦合二元变量与发射协方差，必须采用专门的混合整数优化或逐次凸近似方法处理。基于 Sum Big-M 功率门控的表述，以及将 $\mat{J}_p^{\mathrm{data}}$ 仅视为 $\mat{R}_X$ 仿射函数的推导，对应于不同的建模假设；在完成上述重构之前，它们不能用于证明 \eqref{eq:clustered-fim} 的精确求解。

本章数学推导的原始 (P1) 问题决策变量为 per-AP 波束 $\vect{w}_{m,k} \in \mathbb{C}^{N_t}$、感知协方差 $\mat{Z}_m \in \mathbb{H}_+^{N_t}$、AP-target 软分配权重 $b_{mp} \in [0,1]$，其为上述感知集群二元变量的连续松弛。具体形式为
\begin{align}
...
\end{align}
其中 $\mat{J}_p^{\text{data}}(\mat{R}_X) = \frac{2}{\sigma_s^2} \Real\!\left\{ \mat{D}_p^H \mat{R}_X \mat{D}_p \right\} \in \mathbb{H}_+^{N_\theta \times N_\theta}$ 为等效 MISO 感知观测模型下的数据依赖 Fisher 信息矩阵；$\mat{D}_p = \partial \vect{g}_p / \partial \boldsymbol{\theta}_p \in \mathbb{C}^{MN_t \times N_\theta}$ 为有效目标响应的导数矩阵，$\hat{\mat{H}}_k(\Delta\vect{h}_k) \triangleq (\hat{\vect{h}}_k + \Delta\vect{h}_k)(\hat{\vect{h}}_k + \Delta\vect{h}_k)^H$。该模型直接以发射协方差 $\mat{R}_X$ 描述信息增益，不假设向量化的多接收机观测架构。注意 (P2) 中 $\mat{Z}$ 是 stacked sensing covariance，物理上对应"全部 $M$ 个 AP 都发射 sensing beam"的全局协作假设；$b_{mp}$ 通过 (P2-C4'a)/(P2-C4'b) 间接调节每 AP 的可用功率预算，但 (P2-C2) 感知 SINR 仍按全 AP 协作计算。
...
问题 (P2) 在 lifted 协方差域中重写 (P1)。协方差提升保留了 \S\ref{sec:formulation} 中识别出的六条来源中的五条：(P2-C1) 中的 SINR 分式结构（NC1）；(P2-C5')/(P2-C9) 软分配等式与 box 约束（NC3 + 软化）；(P2-C4'a)/(P2-C4'b) 在对角选择矩阵 $\mat{E}_m$ 下的波束-功率-分配耦合（NC5）。半无限最差情况量词（NC2）在 (P2) 域被改写为关于 $\Delta\vect{h}_k$ 的二次形式，将由 \S\ref{sec:relaxation} Step 3 的 S-Procedure 处理。PCRB 约束 (P2-C3) 在 (P1) 中已含矩阵逆，lifting 后保持 $\tr((\mat{J}_p^{\text{data}})^{-1}) \leq \Gamma_{\text{Track},p}$ 形式，将在 \S\ref{sec:relaxation}-D 由 Schur 补处理。Lifting 新增一项非凸性：rank-1 流形约束（NC4, (P2-C8)）要求 $\mat{W}_k = \vect{w}_k \vect{w}_k^H$。PSD 锥约束 (P2-C6) 与 (P2-C7) 自身是凸的，(P2-C2)（感知 SINR）在 lifted 域通过恒等式 $\vect{g}_p^H \mat{Z} \vect{g}_p = \tr(\vect{g}_p \vect{g}_p^H \mat{Z})$ 变为关于 $\mat{Z}$ 的仿射函数。(P2) 域中残留的非凸/非线性来源由 \S\ref{sec:relaxation} 的凸化链 + \S3 双 DC 惩罚 SCA 框架联合处理。
...
\min \quad \underbrace{\sum_k \tr(\mat{W}_k) + \tr(\mat{Z})}_{\text{原目标}} + \underbrace{\eta_{\text{rank}} \sum_k \rho(\mat{W}_k)}_{\text{秩一惩罚 (DC)}} + \underbrace{\eta_b \sum_{m,p} g(b_{mp})}_{\text{二元惩罚 (DC)}} \quad \text{s.t. (P3-C1)} \sim \text{(P3-C9)}.
\end{equation}
...
\paragraph{关键不等式 (2).} 取迭代点 $(\{\mat{W}_k^{(t)}\}, \{b_{mp}^{(t)}\})$ 作为第 $t$ 次子问题的可行点（约束 (P3-C1)--(P3-C8) 在迭代点处仍满足，因 (P3-C1)--(P3-C8) 关于 $(\mat{W}_k, \mat{Z}, b_{mp})$ 凸、(P3-C9) box 在迭代点满足），其子问题目标值为
\begin{align*}
...
\paragraph{原始约束恢复.} 在 $(\rho, g) = (0, 0)$ 的收敛点，恢复出的 $\vect{w}_k^\star = \sqrt{\lambda_{\max}(\mat{W}_k^\star)} \vect{u}_{\max,k}^\star$、$b_{mp}^\star \in \{0,1\}$ 代入 (P1) 后，所有约束由 (P3) 的凸化等价链严格保持：(P1-C1)/(P1-C2)/(P1-C3) 由 (P3-C1)/(P3-C2)/(P3-C3) + (P3-C3b) 保证；(P1-C4'a)/(P1-C4'b)/(P1-C5')/(P1-C6)/(P1-C7') 由 (P3-C4'a)/(P3-C4'b)/(P3-C5')/(P3-C6)/(P3-C9) 保证；rank-1 性质保证 $\mat{W}_k^\star$ 可恢复为 $\vect{w}_k^\star$。故 (P1-C1)--(P1-C7') 全部满足。
