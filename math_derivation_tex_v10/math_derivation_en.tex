...
\end{equation}
where $\eta_b \geq 0$ is the penalty factor; the objective exhibits a DC structure (``convex function $+$ concave function $\times \eta_b$'') and the box constraints (P2-C9) are kept in their convex form, but the term $-\eta_b b_{mp}^2$ in the objective is concave in $b_{mp}$, so (P2-DCP) is not a convex optimization problem and this non-convexity is handled jointly with the rank-1 constraint by the dual DC penalty SCA framework in \S3. For $b_{mp}\in\{0,1\}$, $g(b_{mp})=0$; for $b_{mp}\in(0,1)$, $g(b_{mp})>0$ and the penalty grows with $\eta_b$. A finite penalty can reduce but does not by itself prove that this residual vanishes; exact recovery must be established by the residual test and fixed-assignment recovery in \S3. After introducing the $\eta_b$ concave penalty, (P2-DCP) has exactly the same mathematical structure as the rank-1 penalty $\rho(\mat{W}_k)=\tr(\mat{W}_k)-\lambda_{\max}(\mat{W}_k)$ introduced in \S3-A---both are ``convex $-$ convex'' DC forms---so the SCA framework designed in \S3 can carry both penalties simultaneously. A large-scale-fading heuristic may be used as a warm start, but for this non-convex SCA it can affect both convergence speed and the stationary point reached.
...
For any positive-semidefinite matrix $\mat{W}_k \succeq \mat{0}$, its trace equals its largest eigenvalue if and only if it has rank at most one.  In the nonzero case (which is implied for an active user by a positive SINR target), this is equivalent to rank one:
...
\label{alg:sca-joint}
\begin{algorithmic}[1]
\Require Fixed penalty factors $\eta_{\text{rank}},\eta_b>0$, tolerance $\varepsilon>0$, and maximum number of iterations $T_{\max}$.
\Ensure An approximately rank-one covariance, a cardinality-feasible binary assignment, and a physically verified recovered solution when recovery succeeds.
\State Solve the unpenalized SDR ($\eta_{\text{rank}}=\eta_b=0$) to obtain a feasible SCA start $(\{\mat{W}_k^{(0)}\},\{b_{mp}^{(0)}\},\ldots)$. Report infeasible if it has no solution. A heuristic may provide a warm start, but may affect the stationary point reached.
\For{$t = 0, 1, \ldots, T_{\max}-1$}
    ...
    \State Evaluate the true fixed-penalty objective $F_\eta^{(t+1)}$ in \eqref{eq:p3-dcp}.
    \If{$\delta_{\text{rank}}^{(t)} \leq \varepsilon$, $\delta_b^{(t)} \leq \varepsilon$, and $|F_\eta^{(t+1)}-F_\eta^{(t)}|\leq\varepsilon$}
        \State \textbf{break}.
    \EndIf
\EndFor
\State For each target $p$, set exactly the $N_{\text{req}}$ largest entries of $\{b_{mp}^{(t+1)}\}_{m=1}^M$ to one and the others to zero, obtaining $b_{mp}^{\rm fix}$.
\State Re-solve the continuous fixed-assignment recovery problem with $b=b^{\rm fix}$, and verify every original physical constraint. If $\delta_{\rm rank}\leq\varepsilon$ and the recovered $\mat{W}_k$ is nonzero, set $\vect{w}_k^\star=\sqrt{\lambda_{\max}(\mat{W}_k)}\vect{u}_{\max,k}$; otherwise declare recovery unsuccessful.
\State \Return only a physically verified recovered solution.
\end{algorithmic}
\end{algorithm}

\begin{theorem}[Dual DC SCA Convergence]\label{thm:sca-conv}
For fixed $\eta_{\text{rank}},\eta_b>0$, exact solution of every convex subproblem, and a feasible initial point, the true fixed-penalty objective $\{F_\eta^{(t)}\}$ generated by Algorithm~\ref{alg:sca-joint} is monotonically non-increasing and bounded below, hence convergent. This statement alone does not assert that a finite penalty makes either residual exactly zero. If an accumulation point has $\rho(\mat{W}_k^\star)=0$ for every nonzero $\mat{W}_k^\star$ and $g(b_{mp}^\star)=0$ for every $(m,p)$, then the rank-one extraction and the cardinality-preserving assignment agree with that point and satisfy the original constraints (P1-C1)--(P1-C7').
\end{theorem}
...
If one pursues the fastest single-SDP run, the master problem (P3) with $\eta_{\text{rank}}=\eta_b=0$ can be solved directly and a rank-one candidate can be extracted from its leading eigenvectors. This is only a baseline: extraction and assignment projection can break the robust-SINR/PCRB guarantees, so every candidate must be checked against the original constraints and an infeasible candidate must be reoptimized or discarded.
...
\Else
    \State Solve a fixed-assignment recovery problem and recheck all original constraints; if it is infeasible, discard this candidate.
\EndIf
\end{algorithmic}
\end{algorithm}

After the convexification chain of \S\ref{sec:relaxation} and the dual DC penalty SCA of \S3, all physical constraints of (P3) (robust SINR, PCRB, sensing SINR, AP power, AP--target assignment, box) are represented by convex constraints. The remaining rank and binarity residuals are controlled in the same SCA loop; their numerical values, the cardinality-preserving projection, and the subsequent physical feasibility check must be reported rather than inferred from an unverified exact-penalty claim.
...
This appendix proves monotonicity of the true fixed-penalty objective; it does not claim exact rank-one or binary recovery for an arbitrary finite penalty.

Let $F_\eta(x)$ denote the objective in \eqref{eq:p3-dcp}, where $x$ collects all decision variables. At iteration $t$, let $Q_t(x)$ be its SCA majorizer obtained by replacing $\rho(\mat{W}_k)$ with \eqref{eq:rho-linearized} and $g(b_{mp})$ with the full affine upper bound in \eqref{eq:g-linearized}, including its constant term. The constants are omitted in \eqref{eq:p3-sca} only because they do not change its minimizer.

For every feasible $x$, convexity of $\lambda_{\max}$ and concavity of $g$ give $F_\eta(x)\leq Q_t(x)$. The two bounds are tight at the current iterate, so $F_\eta(x^{(t)})=Q_t(x^{(t)})$. If $x^{(t+1)}$ minimizes the convex subproblem, then
\[
 F_\eta(x^{(t+1)})\leq Q_t(x^{(t+1)})\leq Q_t(x^{(t)})=F_\eta(x^{(t)}).
\]
Thus $\{F_\eta(x^{(t)})\}$ is monotonically non-increasing. It is bounded below by zero because the power and both deficiency measures are nonnegative; hence it converges.

At any accumulation point for which $\rho(\mat{W}_k^\star)=0$ and $\mat{W}_k^\star\neq0$, $\mat{W}_k^\star$ is rank one and can be represented by its leading eigenvector. If also $g(b_{mp}^\star)=0$, then $b_{mp}^\star\in\{0,1\}$. In this special exact-recovery case, the convexification equivalence chain ensures the original constraints. For finite penalties, residuals must instead be reported and the cardinality-preserving projection followed by fixed-assignment recovery in Algorithm~\ref{alg:sca-joint} must be physically verified.
