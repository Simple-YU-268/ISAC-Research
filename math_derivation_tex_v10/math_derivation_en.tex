...
The decision variables of the original problem (P1) are the per-AP beamforming vectors $\vect{w}_{m,k} \in \mathbb{C}^{N_t}$, the sensing covariance matrices $\mat{Z}_m \in \mathbb{H}_+^{N_t}$, and the AP--target soft-assignment weights $b_{mp} \in [0,1]$ ($b_{mp}=1$ means AP $m$ fully serves target $p$, $b_{mp}=0$ means AP $m$ does not participate in sensing for target $p$; equivalent to the original $\{0,1\}$ binary indicator after the DC relaxation in \S\ref{sec:relaxation}-E):
\begin{align}
...
\end{equation}
where $\mat{D}_p \triangleq \partial \vect{g}_p / \partial \boldsymbol{\theta}_p \in \mathbb{C}^{MN_t \times N_\theta}$ is the partial-derivative matrix of the steering vector and $\otimes$ is the Kronecker product; the CRB matrix is $\mat{C}_p = (\mat{J}_p^{\text{data}})^{-1}$ and (P1-C3) reads $\tr(\mat{C}_p) = \tr((\mat{J}_p^{\text{data}})^{-1}) \leq \Gamma_{\text{Track},p}$, an upper bound on the sum of mean-squared errors of the $N_\theta$-dimensional target state. \eqref{eq:fim-original} is an affine function of $\mat{R}_X$ (since $\mat{R}_X^T \otimes \mat{I}_{N_\theta}$ is affine in $\mat{R}_X^T$ and $\mat{D}_p^H \cdot \mat{R}_X^T \cdot \mat{D}_p$ is affine in $\mat{R}_X$), so $\mat{J}_p^{\text{data}}$ retains the trace-of-inverse form in the lifted domain (P2) and must not be simplified to $\tr(\mat{F}_p \mat{R}_X) \geq \Gamma$ (that shorthand holds only for scalar parameters $N_\theta = 1$, and is invalid for $N_\theta \geq 2$ multi-dimensional target tracking); at $N_\theta = 1$ the FIM degenerates to the scalar $\frac{2}{\sigma_s^2} \|\mat{D}_p^H \mat{R}_X^{1/2}\|^2$ and the constraint reduces to $\mat{J}_p^{\text{data}} \geq 1/\Gamma$ (the v5/v6 shorthand $\tr(\mat{F}_p \mat{R}_X) \geq 1/\Gamma$), which is self-consistent with \eqref{eq:fim-original}. The two coupled power constraints (P1-C4'a)/(P1-C4'b) jointly implement the ``hard-shutdown + full-power release'' physical logic through a Sum Big-M form: when AP $m$ is not assigned to any sensing target ($\sum_{p \in \mathcal{P}^{\text{active}}} b_{mp} = 0$), the right-hand side of (P1-C4'a) collapses to $0$ and physically forces the shutdown of all RF transmission at that AP---$\sum_k \|\vect{w}_{m,k}\|^2 + \tr(\mat{Z}_m) = 0$, corresponding to $\vect{w}_{m,k} = \mat{0}$ and $\mat{Z}_m = \mat{0}$; in the lifted domain (P2) this means $\tr(\mat{E}_m \mat{R}_X) = 0$, so the antenna block of AP $m$ contributes nothing to the global $\mat{R}_X$ and the AP physically disappears from the communication SINR (P1-C1) (its contribution to both numerator and denominator vanishes) as well as from the sensing FIM $\mat{J}_p^{\text{data}}$ (an affine function of $\mat{R}_X$), without any modification to the SINR/FIM formula itself. Conversely, as soon as AP $m$ is selected for at least one target ($\sum_p b_{mp} \geq 1$), the right-hand side of (P1-C4'a) becomes $\geq P_{\max}$ and the constraint is automatically inactive; the standard hardware limit (P1-C4'b) then takes over, allowing AP $m$ to pour up to the full $P_{\max}$ into communication and sensing cooperation. This Sum Big-M formulation embeds AP selection strictly into the physical power budget, is the physical foundation that allows the subsequent dual DC penalty framework to achieve a closed-form mathematical solution, and introduces no new decision variables (the coefficient of $b_{mp}$ in (P1-C4'a) is the constant $P_{\max}$); the \S3 dual DC penalty SCA derivation only needs to update the coefficient of (P1-C4') from $1/N_{\text{req}}$ to $1$ and to add the new standard hardware limit (P1-C4'b).
...
\end{align}
Here $\mat{J}_p^{\text{data}}(\mat{R}_X) = \frac{2}{\sigma_s^2} \Real\!\left\{ \mat{D}_p^H \big( \mat{R}_X^T \otimes \mat{I}_{N_\theta} \big) \mat{D}_p \right\} \in \mathbb{H}_+^{N_\theta \times N_\theta}$ is the data-dependent Fisher Information Matrix ($\mat{D}_p = \partial \vect{g}_p / \partial \boldsymbol{\theta}_p \in \mathbb{C}^{MN_t \times N_\theta}$; see the definition at the end of \S\ref{sec:formulation}) and $\hat{\mat{H}}_k(\Delta\vect{h}_k) \triangleq (\hat{\vect{h}}_k + \Delta\vect{h}_k)(\hat{\vect{h}}_k + \Delta\vect{h}_k)^H$. Note that $\mat{Z}$ in (P2) is the stacked sensing covariance, physically corresponding to the assumption that ``all $M$ APs transmit sensing beams'' in a global-cooperation sense; $b_{mp}$ adjusts the per-AP available power budget indirectly through (P2-C4'a)/(P2-C4'b), while (P2-C2) sensing SINR is still computed with full-AP cooperation (this is the standard cell-free ISAC assumption).
