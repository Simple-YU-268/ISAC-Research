...
\text{s.t.} \quad
& \min_{\|\Delta\vect{h}_k\| \leq \epsilon_h \|\hat{\vect{h}}_k\|}
\frac{|(\hat{\vect{h}}_k + \Delta\vect{h}_k)^H \vect{w}_k|^2}
{\begin{aligned}
&\sum_{j \neq k} |(\hat{\vect{h}}_k + \Delta\vect{h}_k)^H \vect{w}_j|^2 \\
&+ (\hat{\vect{h}}_k + \Delta\vect{h}_k)^H
\big(\textstyle\sum_p \mat{S}_p\big)(\hat{\vect{h}}_k + \Delta\vect{h}_k) + \sigma_c^2
\end{aligned}}
\geq \gamma_k, \quad \forall k, \tag{P1-C1} \\
...
\text{s.t.} \quad
& \begin{aligned}[t]
&\forall \|\Delta\vect{h}_k\| \leq \epsilon_h \|\hat{\vect{h}}_k\|:\quad
\tr(\hat{\mat{H}}_k(\Delta\vect{h}_k)\mat{W}_k)
- \gamma_k \sum_{j \neq k}\tr(\hat{\mat{H}}_k(\Delta\vect{h}_k)\mat{W}_j) \\
&\quad {}- \gamma_k \tr\!\Big(\hat{\mat{H}}_k(\Delta\vect{h}_k)\sum_p\mat{S}_p\Big)
\geq \gamma_k\sigma_c^2,
\end{aligned}\quad \forall k, \tag{P2-C1} \\
...
\begin{equation}
\begin{aligned}
\min \quad &\underbrace{\sum_{k=1}^K \tr(\mat{W}_k) + \sum_{p=1}^P \tr(\mat{S}_p)}_{\text{original objective}}
+ \eta_b \underbrace{\sum_{m \in \mathcal{M}, p \in \mathcal{P}} g(b_{mp})}_{= \sum_{m,p} (b_{mp} - b_{mp}^2)} \\
\text{s.t.}\quad &\text{(P2-C1)} \sim \text{(P2-C9)} \setminus \{\text{(P2-C9)}\}.
\end{aligned}\tag{P2-DCP}
\end{equation}
...
\begin{aligned}
\min \quad &\underbrace{\sum_k \tr(\mat{W}_k) + \sum_p \tr(\mat{S}_p)}_{\text{original objective}}
+ \underbrace{\eta_{\text{rank}} \sum_k \rho(\mat{W}_k)}_{\text{rank-1 penalty (DC)}} \\
&+ \underbrace{\eta_b \sum_{m,p} g(b_{mp})}_{\text{binarity penalty (DC)}} \\
...
\begin{aligned}
\min_{\substack{\{\mat{W}_k\}, \{\mat{S}_p\}, \{\mu_k\}, \\ \{\mat{M}_p\}, \{b_{mp}\}}} \quad
& \sum_{k=1}^K \tr(\mat{W}_k) + \sum_{p=1}^P \tr(\mat{S}_p) \\
& {}+ \eta_{\text{rank}} \sum_{k=1}^K \Big[ \tr(\mat{W}_k)
 - \tr\big(\vect{u}_{\max,k}^{(t)}(\vect{u}_{\max,k}^{(t)})^H\mat{W}_k\big) \Big] \\
& {}+ \eta_b \sum_{m,p}(1 - 2b_{mp}^{(t)})\,b_{mp} \\
\text{s.t.} \quad \text{(P3-C1)}\sim\text{(P3-C9)}.
\end{aligned}
\end{equation}
The objective of \eqref{eq:p3-sca} is linear in $(\{\mat{W}_k\}, \{\mat{S}_p\}, \{b_{mp}\})$. The $\eta_{\text{rank}}$ term combines $\tr(\mat{W}_k)$ with the corresponding negative trace term, while the $\eta_b$ term is linear in $b_{mp}$. Since (P3-C1)--(P3-C9) are LMI, linear, positive-semidefinite, or box constraints, (P3-SCA-$t$) is a standard convex SDP that can be efficiently solved by interior-point solvers such as MOSEK/SDPT3.

Update $(\{\mat{W}_k^{(t+1)}\}, \{b_{mp}^{(t+1)}\}, \{\mat{S}_p^{(t+1)}\}, \{\mu_k^{(t+1)}\}, \{\mat{M}_p^{(t+1)}\})$ and repeat.
