...
class ISAC_v52(nn.Module):
    def __init__(self):
        ...
        self.w_head = nn.Sequential(nn.Linear(64, 128), nn.ReLU(), nn.Linear(128, M * K * Nt), nn.Sigmoid())
...
def compute_sinr_dist(H, w):
    ...
    for k in range(K):
        signal = 0
        for m in range(M):
            signal += np.abs(np.dot(np.conj(w[m, k]), H_complex[m, k])) ** 2
        
        interference = 0
        for j in range(K):
            if j != k:
                for m in range(M):
                    interference += np.abs(np.dot(np.conj(w[m, j]), H_complex[m, k])) ** 2
...
def train_v52(epochs=5000, bs=64, lr=1e-4):
    model = ISAC_v52()
    ...
    for e in range(epochs):
        ...
        loss_comm = F.relu(18 - W_pwr).mean() * 150
        
        # 归一化波束
        w_np = w.detach().numpy()
        w_normalized = w_np / (w_np.sum(axis=(1,2,3), keepdims=True) + 1e-8)
        w_scaled = w_normalized * np.sqrt(Pmax / M / Nt)
        
        sinr_list = []
        for i in range(bs):
            sinr = compute_sinr_dist(H[i], w_scaled[i])
            sinr_list.append(sinr.min())
        sinr_tensor = torch.tensor(sinr_list, dtype=torch.float32)
        
        loss_sinr = F.relu(3 - sinr_tensor).mean() * 80
    ...
    torch.save(model.state_dict(), 'isac_v52.pth')
    print("v52完成!")
    return model

def test_v52(model, n=50):
    model.eval()
    results = []
    ...
    for i in range(n):
        ...
        w_normalized = w_np / (w_np.sum() + 1e-8)
        w_scaled = w_normalized * np.sqrt(Pmax / M / Nt)
        
        W_pwr = np.sum(np.abs(w_scaled) ** 2)
        Z_pwr = (z * b).sum().item() * 3
        total = W_pwr + Z_pwr
        
        sinr_np = compute_sinr_dist(H, w_scaled)
        
        results.append({'Total': total, 'W': W_pwr, 'Z': Z_pwr, 'SINR_min': sinr_np.min()})
    
    print(f"v52: 功率={np.mean([r['Total'] for r in results]):.2f}W")
