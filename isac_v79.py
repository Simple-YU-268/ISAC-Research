...
print("训练...")
m = DeepNet()
o = optim.AdamW(m.parameters(), lr=2e-5, weight_decay=5e-5)
...
torch.save(m.state_dict(), 'isac_v79.pth')
...
print(f"v79 (更深网络, 300测试): SINR_min={min(r):.2f}dB, 平均={np.mean(r):.2f}dB, ≥0dB={sum(1 for x in r if x>=0)}/300")
