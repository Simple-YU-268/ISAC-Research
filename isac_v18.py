...
class ISAC_v18(nn.Module):
    def __init__(self):
        super().__init__()
        hd = cfg.M * cfg.K * cfg.Nt * 2
        self.encoder = nn.Sequential(nn.Linear(hd, 256), nn.LeakyReLU(0.2), nn.BatchNorm1d(256), nn.Dropout(0.1), nn.Linear(256, 128), nn.LeakyReLU(0.2), nn.BatchNorm1d(128), nn.Linear(128, 64))
...
def train_v18(epochs=1000, bs=64, lr=8e-4):
    model = ISAC_v18()
    ...
    for e in range(epochs):
        H = generate_data(bs)
        y_W, y_Z, y_B = generate_labels_v18(bs)
        Xt = torch.tensor(H.reshape(bs, -1), dtype=torch.float32)
        W_ratio, Z_ratio, B = model(Xt)
        loss_W = F.mse_loss(W_ratio.squeeze(), torch.tensor(y_W, dtype=torch.float32).squeeze())
        loss_Z = F.mse_loss(Z_ratio.squeeze(), torch.tensor(y_Z, dtype=torch.float32).squeeze())
        loss_B = F.mse_loss(B, torch.tensor(y_B, dtype=torch.float32))
        ...
        opt.step()
        sched.step()
    torch.save(model.state_dict(), 'isac_v18.pth')
    print(f"v18完成! 用时: {time.time()-t0:.1f}s")
    return model

def test_v18(model, n=20):
    model.eval()
    results = []
    for i in range(n):
        H = generate_data(1)
    ...
    print(f"v18: 功率={np.mean([r['Total'] for r in results]):.1f}W, APs={np.mean([r['APs'] for r in results]):.0f}")
    return results
