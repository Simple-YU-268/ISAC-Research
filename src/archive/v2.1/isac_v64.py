...
def generate_data(n_samples, sigma2=0.3):
    X_list, power_list = [], []
...
print("生成800个训练样本, sigma2=0.3...")
X, P_mmse = generate_data(800, sigma2=0.3)

class ISAC_v64(nn.Module):
    def __init__(self):
        super().__init__()
...
def train_v64(epochs=6000, bs=64, lr=4e-5):
    model = ISAC_v64()
    ...
    for e in range(epochs):
        ...
        loss = F.mse_loss(p, Pt[idx]) * 25
        W_pwr = p.sum(dim=1) * 2.5
        Z_pwr = (z * b).sum(dim=(1,2)) * 2.5
        total_pwr = W_pwr + Z_pwr
        
        loss += F.relu(total_pwr - Pmax).mean() * 600 + F.relu(Pmax * 0.97 - total_pwr).mean() * 500
    ...
    torch.save(model.state_dict(), 'isac_v64.pth')
    print("v64完成!")
    return model

def test_v64(model, n=100):
    model.eval()
    results = []
    sigma2 = 0.3
    ...
    print(f"v64: 功率={np.mean([r['Total'] for r in results]):.2f}W")
