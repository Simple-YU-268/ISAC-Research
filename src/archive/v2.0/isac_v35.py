...
def generate_labels_v35(n):
    W_labels, Z_labels, B_labels = [], [], []
    for i in range(n):
        b = np.random.rand(cfg.M, cfg.P) * 0.028
        for p in range(cfg.P):
            selected = np.random.choice(cfg.M, cfg.N_req, replace=False)
            b[selected, p] = 0.991 + np.random.rand(cfg.N_req) * 0.009
...
def train_v35(epochs=3500, bs=64, lr=2.5e-4):
    model = ISAC_v35()
    opt = optim.AdamW(model.parameters(), lr=lr, weight_decay=1.5e-5)
    sched = optim.lr_scheduler.CosineAnnealingWarmRestarts(opt, T_0=700, T_mult=2)
    
    for e in range(epochs):
        H = generate_data(bs)
        y_W, y_Z, y_B = generate_labels_v35(bs)
        Xt = torch.tensor(H.reshape(bs, -1), dtype=torch.float32)
        
        W_ratio, Z_ratio, B = model(Xt)
        
        loss_sup = F.mse_loss(W_ratio.squeeze(), torch.tensor(y_W, dtype=torch.float32).squeeze()) + F.mse_loss(Z_ratio.squeeze(), torch.tensor(y_Z, dtype=torch.float32).squeeze()) + F.mse_loss(B, torch.tensor(y_B, dtype=torch.float32)) * 0.02
        
        W_pwr = (W_ratio.squeeze() * cfg.Pmax).mean()
        Z_pwr = (Z_ratio.squeeze() * cfg.Pmax).mean()
        total_pwr = W_pwr + Z_pwr
        loss_pwr = F.relu(total_pwr - cfg.Pmax).mean() * 200 + F.relu(cfg.Pmax * 0.99 - total_pwr).mean() * 150 + F.relu(total_pwr - cfg.Pmax * 1.01).mean() * 100
    ...
    torch.save(model.state_dict(), 'isac_v35.pth')
    print("v35完成!")
    return model

def test_v35(model, n=200):
    model.eval()
    results = []
    for i in range(n):
        H = generate_data(1)
    ...
    print(f"v35: 功率={np.mean([r['Total'] for r in results]):.3f}W ± {np.std([r['Total'] for r in results]):.3f}W, APs={np.mean([r['APs'] for r in results]):.0f}")
    return results
