...
def train_v32(epochs=6000, bs=64, lr=2e-4):
    model = ISAC_v32()
    ...
    for e in range(epochs):
        ...
        loss_sup = F.mse_loss(W_ratio.squeeze(), W_target) + F.mse_loss(Z_ratio.squeeze(), Z_target) + F.mse_loss(B, B_target) * 0.01
    ...
    torch.save(model.state_dict(), 'isac_v32.pth')
    print("v32完成!")
    return model

def test_v32(model, n=100):
    model.eval()
    results = []
    for i in range(n):
        H = generate_data(1)
    ...
    print(f"v32: 功率={np.mean([r['Total'] for r in results]):.3f}W ± {np.std([r['Total'] for r in results]):.3f}W, APs={np.mean([r['APs'] for r in results]):.0f}")
    return results
