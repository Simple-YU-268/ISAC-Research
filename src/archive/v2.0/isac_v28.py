...
def train_v28(epochs=6000, bs=64, lr=3e-5):
    model = ISAC_v28()
    ...
    for e in range(epochs):
        H = generate_data(bs)
        y_W, y_Z, y_B = generate_labels_v28(bs)
        Xt = torch.tensor(H.reshape(bs, -1), dtype=torch.float32)
        W_ratio, Z_ratio, B = model(Xt)
        loss = F.mse_loss(W_ratio.squeeze(), torch.tensor(y_W, dtype=torch.float32).squeeze()) + F.mse_loss(Z_ratio.squeeze(), torch.tensor(y_Z, dtype=torch.float32).squeeze()) + F.mse_loss(B, torch.tensor(y_B, dtype=torch.float32)) * 0.005
        total_pwr = (W_ratio.squeeze() + Z_ratio.squeeze()) * cfg.Pmax
        loss = loss + F.relu(total_pwr - cfg.Pmax).mean() * 500 + F.relu(cfg.Pmax - total_pwr).mean() * 400
        opt.zero_grad()
        loss.backward()
        torch.nn.utils.clip_grad_norm_(model.parameters(), 1.0)
        opt.step()
        sched.step()
    torch.save(model.state_dict(), 'isac_v28.pth')
    print("v28完成!")
    return model

def test_v28(model, n=100):
    model.eval()
    results = []
    for i in range(n):
        H = generate_data(1)
    ...
    print(f"v28: 功率={np.mean([r['Total'] for r in results]):.3f}W ± {np.std([r['Total'] for r in results]):.3f}W, APs={np.mean([r['APs'] for r in results]):.0f}")
    return results
