...
def generate_labels(n):
    ...
    for i in range(n):
        ...
        for p in range(cfg.P):
            ...
            b[selected, p] = 0.995 + np.random.rand(cfg.N_req) * 0.005
        W_labels.append(0.757)
        Z_labels.append(0.357)
        B_labels.append(b.flatten())
    return np.array(W_labels).reshape(-1,1), np.array(Z_labels).reshape(-1,1), np.array(B_labels)

class ISAC_v34(nn.Module):
    def __init__(self):
        super().__init__()
...
def train_v34(epochs=8000, bs=64, lr=1.5e-4):
    model = ISAC_v34()
    ...
    for e in range(epochs):
        ...
        loss_sup = F.mse_loss(W_ratio.squeeze(), W_target) + F.mse_loss(Z_ratio.squeeze(), Z_target) + F.mse_loss(B, B_target) * 0.008
        
        W_pwr = (W_ratio.squeeze() * cfg.Pmax).mean()
        Z_pwr = (Z_ratio.squeeze() * cfg.Pmax).mean()
        total_pwr = W_pwr + Z_pwr
        loss_pwr = F.relu(total_pwr - cfg.Pmax).mean() * 500 + F.relu(cfg.Pmax - total_pwr).mean() * 400
    ...
    torch.save(model.state_dict(), 'isac_v34.pth')
    print("v34完成!")
    return model

def test_v34(model, n=100):
    model.eval()
    results = []
    for i in range(n):
        H = generate_data(1)
    ...
    print(f"v34: 功率={np.mean([r['Total'] for r in results]):.3f}W ± {np.std([r['Total'] for r in results]):.3f}W, APs={np.mean([r['APs'] for r in results]):.0f}")
    return results
