...
def train_v39(epochs=3100, bs=64, lr=2.3e-4):
    model = ISAC_v39()
    opt = optim.AdamW(model.parameters(), lr=lr, weight_decay=1.3e-5)
    sched = optim.lr_scheduler.CosineAnnealingWarmRestarts(opt, T_0=620, T_mult=2)
    
    for e in range(epochs):
        H = generate_data(bs)
        y_W, y_Z, y_B = generate_labels_v39(bs)
        Xt = torch.tensor(H.reshape(bs, -1), dtype=torch.float32)
        
        W_ratio, Z_ratio, B = model(Xt)
        
        loss_sup = F.mse_loss(W_ratio.squeeze(), torch.tensor(y_W, dtype=torch.float32).squeeze()) + F.mse_loss(Z_ratio.squeeze(), torch.tensor(y_Z, dtype=torch.float32).squeeze()) + F.mse_loss(B, torch.tensor(y_B, dtype=torch.float32)) * 0.025
        
        W_pwr = (W_ratio.squeeze() * cfg.Pmax).mean()
        Z_pwr = (Z_ratio.squeeze() * cfg.Pmax).mean()
        total_pwr = W_pwr + Z_pwr
        
        loss_pwr = F.relu(total_pwr - cfg.Pmax).mean() * 220 + F.relu(cfg.Pmax * 0.994 - total_pwr).mean() * 170
    ...
    torch.save(model.state_dict(), 'isac_v39.pth')
    print("v39完成!")
    return model

def test_v39(model, n=300):
    model.eval()
    results = []
    for i in range(n):
        H = generate_data(1)
    ...
    print(f"v39: 功率={np.mean([r['Total'] for r in results]):.3f}W ± {np.std([r['Total'] for r in results]):.3f}W, APs={np.mean([r['APs'] for r in results]):.0f}")
    return results
