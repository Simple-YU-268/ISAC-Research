...
def generate_labels_v11(n):
    ...
    for i in range(n):
        ...
        if r < 0.33:
            ...
            b = (np.random.rand(cfg.M, cfg.P) > 0.4).astype(float)
...
def train_v11(epochs=500, bs=64, lr=1e-3):
    ...
    for e in range(epochs):
        H = generate_data(bs)
        y_W, y_Z, y_B = generate_labels_v11(bs)
        ...
        loss_B = F.binary_cross_entropy(B, B_target)
    ...
    torch.save(model.state_dict(), 'isac_v11.pth')
    print(f"\n完成!")
    return model

def test_v11(model, n=20):
    model.eval()
    print("\n测试:")
...
if __name__ == '__main__':
    ...
    model = train_v11(epochs=500, bs=64, lr=1e-3)
    results = test_v11(model, n=20)