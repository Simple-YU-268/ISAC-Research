"""
Cell-free ISAC v37 - 时序优化版本 (马尔可夫目标运动)
...
class CellFreeISACv37:
    ...
    def run_time_frame(self, n_frames=10, T=5):
        """测试时帧优化"""
        print("=" * 80)
        print(f"ISAC v37 - 时帧优化测试 (马尔可夫运动, T={T}时隙)")
        print("=" * 80)
        ...
        for frame in range(n_frames):
            ...
            # 时帧优化 (独立优化每个时隙)
            W_all, Z_all, a_all, rho_all = self.optimize_time_frame(
                H_est_all, G_est_all, H_all, G_all
            )
...
if __name__ == "__main__":
    ...
    # 测试时帧优化
    isac.run_time_frame(n_frames=20, T=5)
