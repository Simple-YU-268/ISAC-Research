...
class CellFreeISACv28:
    ...
    def compute_snr(self, G_sel, Z):
        """计算感知SNR - 考虑双站雷达模型"""
        M_sel, P, Nt = G_sel.shape
        # 简化的CRB公式: SNR = ||g^H Z g||² / σ²
        snrs = []
        for p in range(P):
            signal_power = 0
            for m in range(M_sel):
                g = G_sel[m, p, :]
                # 信号: g^H Z g (协方差矩阵的 quadratic form)
                if Z[m].shape == (Nt, Nt):
    ...
    def run(self, n_sens_list=[4, 8, 16, 32], n_trials=50):
        print("=" * 70)
        print("ISAC v28 - 64 APs: 全AP通信 + 选择性感知")
        print("=" * 70)
        print(f"配置: {self.M} APs 全部参与通信")
        print(f"感知: 选择最佳 AP 子集")
        print(f"功率预算: {self.Pmax}W")
        print(f"约束: SINR≥{self.sinr_req}dB, SNR≥{self.snr_req}dB")
        print("=" * 70)
        
        for N_sens in n_sens_list:
            print(f"\n--- 感知AP数: {N_sens} ---")
            ...
            for i in range(n_trials):
                ...
                # 通信: 所有64 APs参与
                W = self.mmse_beam_all_aps(H_true, self.Pmax * 0.7)
                
                # 感知: 选择最佳 N_sens 个AP
                sens_mask = self.select_sensing_aps(G_true, N_sens)
                G_sel = G_true[sens_mask]
                Z = self.sensing_beam(G_sel, self.Pmax * 0.3)
...
if __name__ == "__main__":
    ...
    isac.run(n_sens_list=[n_ap_default], n_trials=30)
