"""
Cell-free ISAC v41 - Optimized PC-CRLB
PC-CRLB多步预测与自适应优化
...
class CellFreeISACv41:
    ...
    def compute_multi_step_pc_crlb(self, trajectory, W, a, n_steps=2):
        """
        多步PC-CRLB预测
        
        预测n_steps步之后的CRB，更保守地满足约束
        ...
        for step in range(1, n_steps + 1):
            ...
            if len(trajectory) >= 2:
                ...
                predicted_pos = trajectory[-1] + step * velocity
            else:
                predicted_pos = trajectory[-1]
            
            # 边界检查
            predicted_pos = np.clip(predicted_pos, -180, 180)
            
            # 生成预测信道
            _, G_pred = self.generate_channels_at_positions(predicted_pos, None)
            
            # 计算该步的CRB
            crb = self.compute_pc_crlb(G_pred, W, a)
            pc_crlbs.append(crb)
        
        # 返回多步平均 (越远的预测权重越低)
        weights = np.array([1.0 / (i+1) for i in range(n_steps)])
        weights = weights / weights.sum()
        
        avg_crb = sum(w * c for w, c in zip(weights, pc_crlbs))
        
        return avg_crb
    
    def compute_adaptive_pc_crlb_threshold(self, trajectory):
        """
        自适应PC-CRLB阈值
        ...
        """
        if len(trajectory) < 2:
            return self.crb_req  # 无历史数据，使用默认
        
        # 计算速度变化 (不确定性指标)
        velocities = trajectory[1:] - trajectory[:-1]
        velocity_mag = np.linalg.norm(velocities, axis=1)
        
        if len(velocity_mag) > 1:
            # 速度变化大 = 不确定性强
            velocity_variance = np.var(velocity_mag)
        else:
            velocity_variance = 0
        
        # 自适应阈值: 方差大时，使用更严格的CRB要求
        # 基础阈值1m，方差大时降至0.5-0.8m
        base_threshold = self.crb_req
        adaptation_factor = 1.0 - 0.3 * min(velocity_variance / 100, 1.0)  # 最大降低30%
    ...
    def sca_optimize_pc_crlb_v2(self, H_est, G_est, H_true, G_true, G_pred, trajectory=None, n_steps=2):
        """
        优化版PC-CRLB SCA
        ...
        if trajectory is not None:
            adaptive_crb = self.compute_adaptive_pc_crlb_threshold(trajectory)
        else:
            adaptive_crb = self.crb_req
        ...
        for n in range(2, 21):
            ...
            # 尝试不同的功率分配 (感知优先级)
            rho_options = [0.5, 0.4, 0.6, 0.3, 0.7]
            ...
            for rho in rho_options:
                ...
                for _ in range(3):
                    ...
                    # 多步PC-CRLB
                    if trajectory is not None and n_steps > 1:
                        pc_crlb = self.compute_multi_step_pc_crlb(trajectory, W, a, n_steps)
                    else:
                        pc_crlb = self.compute_pc_crlb(G_pred, W, a)
                    
                    # 验证约束
                    v = self.compute_violation(H_true, G_true, a, W, Z, margin_db=0)
                    pc_crlb_ok = np.all(pc_crlb <= adaptive_crb)
    ...
    def optimize_time_frame_kalman(self, H_all, G_all, H_true_all, G_true_all, trajectory, user_pos):
        ...
        for t in range(T):
            ...
            # 优化版PC-CRLB指导优化 (v41核心)
            trajectory_so_far = trajectory[:t+1]  # 到当前时刻的历史
            W_t, Z_t, a_t, rho_t = self.sca_optimize_pc_crlb_v2(
                H_all[t], G_all[t], 
    ...
    def run_time_frame(self, n_frames=10, T=5):
        """测试时帧优化"""
        print("=" * 80)
        print(f"ISAC v41 - Optimized PC-CRLB (多步预测+自适应阈值)")
        print("=" * 80)
