"""
Cell-free ISAC v40 - Multi-AP Collaborative Tracking
多AP协作感知与预测波束成形
...
目标：多AP协作EKF融合，提升预测精度

算法：分布式EKF融合 + PC-CRLB + 协作波束优化

文献参考:
- Cooperative Sensing-Assisted Predictive Beam Tracking (arXiv 2025)
- Multi-BS Kalman Fusion for ISAC (arXiv 2025)
"""
...
class CellFreeISACv40:
    ...
    def multi_ap_kalman_fusion(self, trajectory, active_aps, dt=1.0):
        """
        多AP协作EKF融合预测
        ...
        Args:
            trajectory: (t, P, 2) 历史轨迹
            active_aps: (M,) AP激活状态
            dt: 时间间隔
            
        Returns:
            predicted_pos: (P, 2) 融合预测位置
        """
        ...
        # 过程噪声
        Q = np.eye(4) * 0.05  # 稍微增大过程噪声以应对不确定性
        
        # 多AP观测噪声 (基于AP位置精度)
        # 越靠近目标的AP，观测噪声越小
        active_indices = np.where(active_aps > 0.5)[0]
        ...
        for p in range(P):
            # 初始化状态
            if t == 0:
                x = np.array([trajectory[0, p, 0], trajectory[0, p, 1], 0, 0])
            else:
                # 用最近两个点估计速度
                if t >= 2:
                    vx = (trajectory[-1, p, 0] - trajectory[-2, p, 0]) / dt
                    vy = (trajectory[-1, p, 1] - trajectory[-2, p, 1]) / dt
                else:
                    vx, vy = 0, 0
                x = np.array([trajectory[-1, p, 0], trajectory[-1, p, 1], vx, vy])
            
            P_cov = np.eye(4) * 50  # 初始协方差
            
            # 对每个活跃AP进行测量更新
            for ap_idx in active_indices[:min(n_active, 5)]:  # 最多用5个AP
                # 计算AP到目标的距离 (模拟测量精度)
                if t > 0:
                    ap_pos = self.ap_pos[ap_idx]
                    target_pos = trajectory[-1, p]
                    distance = np.linalg.norm(ap_pos - target_pos)
                    
                    # 距离越近，观测噪声越小 (5m -> R=5, 100m -> R=50)
                    R_ap = np.eye(2) * max(5, min(50, distance / 2))
                else:
                    R_ap = np.eye(2) * 20  # 默认噪声
                ...
                # 更新步骤 (融合该AP的测量)
                if t > 0:
                    # 使用真实位置作为测量 (无噪声，仅测试融合架构)
                    z = trajectory[-1, p]
                    
                    y = z - H_obs @ x_pred  # 残差
                    S = H_obs @ P_pred @ H_obs.T + R_ap
        ...
        for t in range(T):
            ...
            if t == 0:
                ...
                if a_prev is not None:
                    # 使用上一时隙的AP选择进行多AP融合
                    predicted_pos = self.multi_ap_kalman_fusion(observed, a_prev)
                else:
                    # 回退到单AP预测
    ...
    def run_time_frame(self, n_frames=10, T=5):
        """测试时帧优化"""
        print("=" * 80)
        print(f"ISAC v40 (Multi-AP) - 多AP协作EKF预测优化 (马尔可夫运动, T={T}时隙)")
        print("=" * 80)
...
if __name__ == "__main__":
    ...
    # 测试多AP协作EKF预测 (T=3)
    print("\n" + "="*80)
    print("Multi-AP Collaborative Tracking - 多AP协作EKF预测 (T=3)")
    print("="*80)
