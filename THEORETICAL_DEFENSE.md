...
> 
> 在论文中明确声明：
> - SDR 提供原问题下界——per-user SINR + cell-free + 鲁棒问题**无通用紧致性定理**（multicast $G_k \leq 2$ / robust MISO $N_t \leq 2$ 紧致条件不直接套用）
> - 高斯随机化提供可行 rank-1 解（**无紧致性能损失上界**；实际工程用 $L = 100 \sim 1000$ 候选并选最优）
