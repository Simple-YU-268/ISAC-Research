...
本文档给出了原问题 (P1) 的定义与约束分析，但**逐步凸化推导链**（每步的「为什么非凸 → 怎么变 → 等价还是上下界 → 最终凸形式」）已抽离为独立文档：

> 📄 **`CONVEXIFICATION_CHAIN.md`** —— 7 步凸化方法链 + 每步命题证明 + 最终凸 SDP (P3)

该文档专为回应"逐步凸化推导 + 等价性证明"的审稿质疑而写，结构为：
1. 原问题编号回顾 (§1, 对应 (5a)-(5h))
2. 6 个非凸源识别 (§2)
3. 7 步凸化方法链 (§3, 每步含命题证明)
4. 最终凸形式 P3 (§4)
5. 等价性总结表 (§5)
6. 复杂度对比 (§6)

**答辩时优先引用**: `CONVEXIFICATION_CHAIN.md §8`（导师问题的简明答案）。
