% Cell-Free ISAC Complete MATLAB Solver v2 - Optimized
% Improvements: sensing-aware AP selection, adaptive rho, joint beamforming
...
function selected = selectApsV2(cfg, scenario, nActive, method)
    Hhat = scenario.Hhat; Ghat = scenario.Ghat;
    switch method
        case 'joint_score'
            commScore = zeros(cfg.M, 1); sensScore = zeros(cfg.M, 1);
            for m = 1:cfg.M
        ...
        case 'sensing_aware'
            ...
            for p = 1:cfg.P
                ...
                score(order(1:min(nPerTarget, cfg.M))) = score(order(1:min(nPerTarget, cfg.M))) + 1;
            end
...
function compareWithV1()
    fprintf('\n============================================================\n');
    fprintf('Comparison with v1\n');
    fprintf('============================================================\n');
    fprintf('v1 baseline: 50% success rate\n');
    fprintf('v2 improvements:\n');
    fprintf('  1. Sensing-aware AP selection (target-specific grouping)\n');
    fprintf('  2. Adaptive rho search based on channel conditions\n');
    fprintf('  3. Joint sensing beam (orthogonal to communication)\n');
    fprintf('  4. Iterative power refinement\n');
    fprintf('  5. Multiple selection strategies (joint/sensing-aware/comm-first)\n');
end
