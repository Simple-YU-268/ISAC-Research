% Cell-Free ISAC Solver v3 - Smart Search
% Based on v1 insight: success comes from 6-12 APs, not 16
...
fprintf('============================================================\n');
fprintf('Cell-Free ISAC Solver v3 (Smart Search)\n');
fprintf('============================================================\n');
...
function cfg = defaultConfig()
    cfg.seed = 42; cfg.M = 16; cfg.Nt = 4; cfg.K = 10; cfg.P = 4;
    cfg.apGridSide = 4; cfg.apMin = -60; cfg.apMax = 60;
    cfg.userMin = -50; cfg.userMax = 50; cfg.targetMin = -30; cfg.targetMax = 30;
    ...
    cfg.nActiveCandidates = [6 8 10 12 14 16 4];  % v3: try 6-12 first
...
function result = emptyResult()
    result.success = false; result.nActive = 0; result.rho = NaN;
    result.selected = []; result.minSinrDb = -Inf; result.minSinrWcDb = -Inf;
    result.minSnrDb = -Inf; result.minSnrWcDb = -Inf; result.maxCrb = Inf;
    result.totalPower = Inf; result.maxApPower = Inf; result.violation = Inf;
    result.sinrDb = []; result.sinrWcDb = []; result.snrDb = [];
    result.snrWcDb = []; result.crb = []; result.apPower = [];
end
...
function best = solveOneScenario(cfg, scenario)
    ...
    for nActive = cfg.nActiveCandidates
        selected = selectAps(cfg, scenario.Hhat, scenario.Ghat, nActive);
...
function selected = selectAps(cfg, Hhat, Ghat, nActive)
...
function summarizeResults(cfg, results)
    ...
    fprintf('\n============================================================\n');
    fprintf('Summary v3 (Smart Search)\n');
    fprintf('============================================================\n');
    ...
    fprintf('\nv3 vs v1: Same algorithm, but search order optimized\n');
    fprintf('v1: [4 6 8 10 12 16] -> 50% success\n');
    fprintf('v3: [6 8 10 12 14 16 4] -> tries sweet spot first\n');
end
